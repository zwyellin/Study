...
Ext.extend(Ext.ux.FileUploader, Ext.util.Observable, {
	...
	,processSuccess:function(options, response, o) {
		...
		if(this.singleUpload) {
			...
			record.set('shortName',"<a href='#' onclick=\"downloadAttachment('" + o.aid + "')\">" + shortName + "</a>");
			record.set('aid',o.aid);
			record.set('error', '');
			record.commit();
			if(this.uploadPanel.attachIdFieldName){
				var ele = document.getElementsByName(this.uploadPanel.attachIdFieldName)[0];
				if(ele.value && ele.value!=''){
					ele.value = ele.value + ',' + o.aid;
				} else{
					ele.value = o.aid;
				}
			}
	...
	,uploadCallback:function(options, success, response) {
		this.uploadPanel.getTopToolbar().setVisible(true);
		this.uploadPanel.progressBar.updateProgress(0,' ');
		this.uploadPanel.progressBar.setVisible(false);
		this.queryProgressBarFlag = false;
		...
		this.form = false;
 		...
 		if(true === success) {
			try {
  				o = Ext.decode(response.responseText.replace(/<\/?pre>/ig, ''));
 				o.success = true;
 			}
			catch(e) {
	...
	,uploadFile:function(record, params) {
		...
		// request upload
		Ext.Ajax.request(o);
		this.queryProgressBarFlag = true;
		this.uploadPanel.getTopToolbar().setVisible(false);
		this.uploadPanel.progressBar.setVisible(true);
		this.queryProgressBarCount.defer(1000, this);			                	
	...
	,queryProgressBarCount:function() {  
			                    	Ext.Ajax.request({  
			                                url : '../../Attachment_queryUploadBar_1_1.action?p=' + new Date(),
			                                method : 'get',  
			                                //处理ajax的返回数据  
			                                scope : this,
			                                success :  this.queryProgressBarCallBack
			                            });  
	   
	}
	,queryProgressBarCallBack:function(response, options) {  
			                                    var obj = Ext.util.JSON.decode(response.responseText);  
			                                    if (obj && obj.pBytesRead && obj.pContentLength) {  
			                                            this.uploadPanel.progressBar.updateProgress((obj.pBytesRead/obj.pContentLength),(obj.pBytesRead + "/" + obj.pContentLength));  
			                                   }  			 
												if(this.queryProgressBarFlag) this.queryProgressBarCount.defer(1000, this);			                	
