/*
 * Extending/depending on: ~ = modified function (when updating from SVN be sure
 * to check these for changes, especially to Ext.tree.TreeNodeUI.render() ) + =
 * added function
 * 
 * TreeSelectionModel.js Ext.tree.CheckNodeMultiSelectionModel : ~init(),
 * ~onNodeClick(), +extendSelection(), ~onKeyDown()
 * 
 * TreeNodeUI.js Ext.tree.CheckboxNodeUI : ~render(), +checked(), +check(),
 * +toggleCheck()
 * 
 * TreePanel.js Ext.tree.TreePanel : +getChecked()
 * 
 * TreeLoader.js Ext.tree.TreeLoader : ~createNode()
 * 
 */

/**
 * 原始功能只能实现选中或不选两种状态 但我们现在需要非叶子节点可以实现三种状态，既： 节点的子节点都被选中时，显示都被选中的图标
 * 节点的子节点都没选中时，显示都没选中的图标 节点的子节点部分选中时，显示部分选中的图标
 * 
 * 显示都被选中，和部分选中时，当前节点的状态是被选中 显示都没选中时，当前节点的状态是没选中
 * 
 ...
 */
/*
 * Ext.tree.TreePanel.prototype.getChecked = function(node){ var checked = [],
 * i; if( typeof node == 'undefined' ) { node = this.rootVisible ?
 * this.getRootNode() : this.getRootNode().firstChild; }
 * 
 * if( node.attributes.checked ) { checked.push(node.id); if( !node.isLeaf() ) {
 * for( i = 0; i < node.childNodes.length; i++ ) { checked = checked.concat(
 * this.getChecked(node.childNodes[i]) ); } } } return checked; };
 */

/**
 * @class Ext.tree.CustomUITreePanel
 * @extends Ext.tree.TreePanel
 * 
 * 重写getChecked()使其达到如下效果： 原始功能只能实现选中或不选两种状态 但我们现在需要非叶子节点可以实现三种状态，既：
 * 节点的子节点都被选中时，显示都被选中的图标 节点的子节点都没选中时，显示都没选中的图标 节点的子节点部分选中时，显示部分选中的图标
 * 
 * 显示都被选中，和部分选中时，当前节点的状态是被选中 显示都没选中时，当前节点的状态是没选中
 * 
 * 显示都被选中时，单击当前节点，触发的事件是，当前节点变成未选中状态，所有子节点变成未选中状态
 ...
 * @return {Array} 选中节点的id数组
 */
Ext.tree.CustomUITreePanel = function() {
	Ext.tree.CustomUITreePanel.superclass.constructor.apply(this, arguments);
}
Ext.extend(Ext.tree.CustomUITreePanel, Ext.tree.TreePanel, {
	getChecked : function(node) {
		var checked = [], i;
		if (typeof node == 'undefined') {
			node = this.rootVisible
			? this.getRootNode()
 ...
 * @extends Ext.tree.TreeLoader 重写createNode()，强制uiProvider是任意的TreeNodeUI来保存广度
 */
 ...
 * @extends Ext.tree.TreeNodeUI 给所有节点添加checkbox
 */
...
Ext.extend(Ext.tree.CheckboxNodeUI, Ext.tree.TreeNodeUI, {
	...
	renderElements : function(n, a, targetNode, bulkRender) {
		...
		/*
		 * var buf = ['<li class="x-tree-node"><div
		 * ext:tree-node-id="',n.id,'" class="x-tree-node-el x-tree-node-leaf
		 * x-unselectable ', a.cls,'" unselectable="on">', '<span
		 * class="x-tree-node-indent">',this.indentMarkup,"</span>", '<img
		 * src="', this.emptyIcon, '" class="x-tree-ec-icon x-tree-elbow" />', '<img
		 * src="', a.icon || this.emptyIcon, '" class="x-tree-node-icon',(a.icon ? "
		 * x-tree-node-inline-icon" : ""),(a.iconCls ? " "+a.iconCls : ""),'"
		 * unselectable="on" />', cb ? ('<input class="x-tree-node-cb"
		 * type="checkbox" ' + (a.checked ? 'checked="checked" />' : '/>')) :
		 * '', '<a hidefocus="on" class="x-tree-node-anchor" href="',href,'"
		 * tabIndex="1" ', a.hrefTarget ? ' target="'+a.hrefTarget+'"' : "", '><span
		 * unselectable="on">',n.text,"</span></a></div>", '<ul class="x-tree-node-ct" style="display:none;"></ul>', "</li>"].join('');
		 */
		...
		var buf = ['<li class="x-tree-node"><div ext:tree-node-id="', n.id,
				...
				a.icon || this.emptyIcon, '" class="x-tree-node-icon',
	...
	check : function(forParent, forChildren) {
		...
		flag = (cls == "x-tree-node-checkbox-none")
				? "x-tree-node-checkbox-all"
				: "x-tree-node-checkbox-none";
		/*
		 * if (this.node.isLeaf) { flag = (cls == "x-tree-node-checkbox-all") ?
		 * "x-tree-node-checkbox-none" : "x-tree-node-checkbox-all"; } else { if
		 * (cls == 'x-tree-node-checkbox-none') { // 全部选中 flag =
		 * 'x-tree-node-checkbox-all'; } else { // 全部反选 flag =
		 * 'x-tree-node-checkbox-none'; } }
		 */

		if (typeof forParent == "undefined" || forParent == null) {
			forParent = (typeof this.node.parentNode != "undefined")
		...
		if (!n.isLeaf && !n.expanded && !n.childrenRendered) {
			n.expand(false, false, this.check.createDelegate(this, [forParent,
					forChildren]));
			return;
		}
		...
		// this.checkboxImg.className = "x-tree-node-checkbox-" +
		// this.isAllChildrenChecked();
	...
	getNextStatus : function() {
		return this.getClassName() == 'x-tree-node-checkbox-none'
				...
				: 'x-tree-node-checkbox-none';
		/*
		 * if (this.isLeaf()) { return this.getClassName() ==
		 * "x-tree-node-checkbox-all" ? "x-tree-node-checkbox-none" :
		 * "x-tree-node-checkbox-all"; } else { if (this.getClassName() ==
		 * "x-tree-node-checkbox-none") { return "x-tree-node-checkbox-all"; }
		 * else { return "x-tree-node-checkbox-none"; } if (this.getClassName() ==
		 * "x-tree-node-checkbox-all") { return "x-tree-node-checkbox-none"; }
		 * else if (this.getClassName() == "x-tree-node-checkbox-none") { return
		 * "x-tree-node-checkbox-some"; } else { return
		 * "x-tree-node-checkbox-all"; } }
		 */
	}
			...
			for (var i = 0; i < children.length; i++) {
				...
				if (c.leaf) {
					...
					if (this.alreadyCheckAllChildren(c.children, c.checked) == 'all') {
						all++;
					} else if (this.alreadyCheckAllChildren(c.children,
							c.checked) == 'some') {
						some++;
					}
				}
	...
	check : function() {
		...
		this.node.attributes.checked = "x-tree-node-checkbox-none" != this
				.getClassName();
		if (this.node.parentNode.getUI().checkParent) {
			this.node.parentNode.getUI().checkParent();
		}
		if (!this.isLeaf()) {
			...
			for (var i = 0; i < children.length; i++) {
				children[i].getUI()
						.checkChild("x-tree-node-checkbox-all" == this
								.getClassName());
			}
		}
	}
	...
	checkChild : function(isCheck) {
		if (this.isLeaf()) {
			this.checkboxImg.className = isCheck
					? "x-tree-node-checkbox-all"
					: "x-tree-node-checkbox-none";
 ...
 * @extends Ext.tree.MultiSelectionModel Multi selection for a TreePanel
 *          containing Ext.tree.CheckboxNodeUI. Adds enhanced selection routines
 *          for selecting multiple items and key processing to check/clear
 *          checkboxes.
 */
...
Ext.extend(Ext.tree.CheckNodeMultiSelectionModel, Ext.tree.MultiSelectionModel,
		{
			...
			/**
			 * Handle a node click If ctrl key is down and node is selected will
			 * unselect the node. If the shift key is down it will create a
			 * contiguous selection (see
			 * {@link Ext.tree.CheckNodeMultiSelectionModel#extendSelection} for
			 * the limitations)
			 */
			...
			/**
			 * Selects all nodes between the previously selected node and the
			 * one that the user has just selected. Will not span multiple
			 * depths, so only children of the same parent will be selected.
			 */
			extendSelection : function(node) {
				...
				if (node.parentNode == last.parentNode) {
					...
					for (i = 0; i < cs.length; i++) {
						// We have to traverse the entire tree b/c don't know of
						// a way to find
						// a numerical representation of a nodes position in a
						// tree.
						if (cs[i].attributes[attr] == last.attributes[attr]
								|| cs[i].attributes[attr] == node.attributes[attr]) {
							// lastSelect ensures that we select the final node
							// in the list
						...
						if (selecting || lastSelect) {
							...
							// if we're selecting the last node break to avoid
