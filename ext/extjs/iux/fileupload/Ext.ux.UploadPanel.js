...
Ext.ux.UploadPanel = Ext.extend(Ext.Panel, {
	...
	,initComponent:function() {
		...
		var addCfg = {
			 xtype:'browsebutton'
			,text:this.addName||this.addText
		...
		if(('body' !== this.buttonsAt) && !this.readonly) {
		...
		var fields = [
			...
			,{name:'aid', type:'text', system:true}			
		...
		// create view
		Ext.apply(this, {
			items:[new Ext.ProgressBar({width:this.width - 5,hidden:true}),{
				 xtype:'dataview'
...
//					+ '<div class="ux-up-indicator">&#160;</div>'
					...
					+ '<div class="ux-up-text" qtip="{fileName}">{shortName}</div>'
					+ '<div class="ux-up-icon-state ux-up-icon-{state}"'
		...
		this.progressBar = this.items.itemAt(0);
		...
		this.uploader.uploadPanel = this;
		if(this.moduleId){
			Ext.Ajax.request({  
			   url : './Attachment!queryAttachments.action',
			   params : {
			   		ran : Math.random(),
			   		moduleId : this.moduleId,
			   		datatype : this.datatype
			   ...
			   method : 'post',  
			   scope : this,
			   success : this.onGetAttachmentRelation
            });  			
		}
	...
	,onGetAttachmentRelation:function(response, options){
		var obj = Ext.util.JSON.decode(response.responseText);
		var attList = obj;
		if(this.hiddenforcount){
		...
		for(var i=0;i<attList.length;i++){
			...
			var rec = new this.store.recordType({
				...
				,shortName: "<a href='#' onclick=\"downloadAttachment('" + att.attachmentId + "')\">" + att.realname + "</a>"
				,fileCls:this.getFileCls(att.realname)
				,state:'done'
			}, att.attachmentId);
			rec.commit();
			this.store.add(rec);
	...
	,onAddFile:function(bb) {
		...
		if(fileName.indexOf(".")!=-1){
			...
			filesuf = d[d.length-1];
			filesuf = filesuf.toLowerCase();
		}
		if(this.suffix){
			if(!filesuf || this.suffix.indexOf(filesuf) ==-1){
				alert("只可上传后缀名为" + this.suffix + "的文件！");			
				return;
			}
		}
	...
	,refreshValue:function(){
		var ele = document.getElementsByName(this.attachIdFieldName)[0];
		var v = '';
		for(var i=0;i<this.store.getCount();i++){
			var r = this.store.getAt(i);
			if(r.get('aid')){
				v = v + r.get('aid') + ((i == this.store.getCount() - 1) ? "" : ",");
			}
		}
		ele.value = v;
	}
	...
	,onRemoveClick:function(btn){
		if(this.view.getSelectedNodes().length!=0){
			var selects = this.view.getSelectedRecords();
			for(var i=0;i<selects.length;i++){
				// remove DOM elements
				var select = selects[i];
				Ext.Ajax.request({  
				   url : './Attachment!deleteAttachment.action',
				   params  : {
				   		aid : select.get('aid'),
				   		datatype : this.datatype,
				   		ownerDelete : this.ownerDelete,
				   		mid : this.moduleId
				   },
				   scope : this,
				   success : function(response){
				   	if(response && response.responseText == "false"){
				   		alert("只有上传人才可以删除此附件！");
				   		return;
				   	}
	            
					var inp = select.get('input');
					if(inp){
						var wrap = inp.up('em');
						inp.remove();
						if(wrap) {
					...
					// remove record from store
					this.store.remove(select);
					
					if(this.attachIdFieldName) this.refreshValue();
	...
	,setHiddenforcount:function(b){
		this.hiddenforcount = b;
	}
