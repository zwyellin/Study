 ...
 *       <ul>
 *       <li><a
 *       href="http://lovcombo.extjs.eu/lovcombo.tar.bz2">lovcombo.tar.bz2</a></li>
...
Ext.ux.form.LovCombo = Ext.extend(Ext.form.ComboBox, {
	...
	/**
	 * @cfg {String} checkField Name of field used to store checked state. It is
	 *      automatically added to existing fields. (defaults to "checked" -
	...
	/**
	 * @cfg {String/Array} tpl Template for items. Change it only if you know
	 *      what you are doing.
	 */
	// }}}
	// {{{
	...
	initComponent : function() {
		...
		if (!this.tpl) {
			this.tpl = '<tpl for=".">' + '<div class="x-combo-list-item">'
					...
					+ (this.displayField || 'text') + ':htmlEncode}</div>'
					+ '</div>' + '</tpl>';
		}
	...
	// {{{
	/**
	 * Sets the value of the LovCombo. The passed value can by a falsie (null,
	 * false, empty string), in which case the combo value is cleared or
	 * separator separated string of values, e.g. '3,5,6'.
	 * 
	 * @param {Mixed}
	 *            v value
	 */
